...
class Devicestatus {
  ...
  static find(sort) {
    let promise = new Promise((resolve, reject) => {
      Devicestatus.model.find({}).sort(sort).exec().then((devicestatuses) => {
        let returnData = []
        let enrichDataPromises = []
        devicestatuses.forEach((devicestatus) => {
          let document = Object.assign({}, devicestatus._doc)
          enrichDataPromises.push(Devicestatus.enrichData(document))
        })
  ...
  static findById(id) {
    console.log('devicestatus findById')
    let promise = new Promise((resolve, reject) => {
      Devicestatus.model.findById(id).exec().then((devicestatus) => {
        let document = Object.assign({}, devicestatus._doc)
        Devicestatus.enrichData(document).then((returnDocument) => {
  ...
  static filter(filterString, sort) {
    console.log('devicestatus filter')
    ...
    let promise = new Promise((resolve, reject) => {
      Devicestatus.model.find({[filterKey]: {$in: filterValues}}).sort(sort).exec().then((devicestatuses) => {
        let returnData = []
        let enrichDataPromises = []
        devicestatuses.forEach((devicestatus) => {
          let document = Object.assign({}, devicestatus._doc)
  ...
  let promise = new Promise((resolve, reject) => {
    Devicestatus.getSelectItems(document).then((values) => {
  ...
  static getSelectItems(baseModel) {
    console.log('devicestatus getSelectItems')
    let promise = new Promise((resolve, reject) => {
  ...
  static getEmbeddedItems(id) {
    console.log('devicestatus getEmbeddedItems')
    let promise = new Promise((resolve, reject) => {
  ...
  static search(config) {
    console.log('devicestatus search')
    let devicestatuses = config.devicestatuses
    ...
    let promise = new Promise((resolve, reject) => {
      let returnData = {
        info: {
          totalCount: devicestatuses.length
        },
        documents: []
      }
      ...
      if(searchString.length > 0) {
        ...
        if(jsFilter) {
          ...
          returnData.documents = devicestatuses
        }
      }
  ...
  static create(postData) {
    console.log('devicestatus create')
    let promise = new Promise((resolve, reject) => {
      let newDevicestatus = Devicestatus.model(postData)
      newDevicestatus.save().then((devicestatus) => {
        ...
        if(postData.hasOwnProperty('<%- parentItem.name %>Id')) {
          let joinData = {
            ...
            devicestatusId: devicestatus._id
          }
          let new<%- parentItem.className %>_devicestatus = <%- parentItem.className %>_devicestatus.model(joinData)
          new<%- parentItem.className %>_devicestatus.save().then((<%- parentItem.name %>_devicestatus) => {
  ...
  static update(id, postData) {
    console.log('devicestatus update')
    let filter = {
      "_id": id
    }
    let promise = Devicestatus.model.findOneAndUpdate(filter, postData).exec()

    return(promise)
  }

  static remove(id) {
    console.log('devicestatus remove')
    let filter = {
      "_id": id
    }
    let promise = Devicestatus.model.findOneAndRemove(filter).exec()
...
Devicestatus.model = mongoose.model('Devicestatus', devicestatusSchema)
